...
def get_data(args):
    ...
    speech_dir = os.path.join(workspace, args.speech_dir, "%s" % args.mode)
    noise_dir = os.path.join(workspace, args.noise_dir, "%s" % args.mode)
    data_type = args.mode
    ...
    for speech_na in speech_names:
        ...
        for noise_na in selected_noise_names:
            ...
            # Truncate noise to the same length as speech.
                ...
                noise_audio = noise_audio[noise_onset: nosie_offset]
            ...
            # Extract spectrogram.
            mixed_complx_x = calc_sp(args, mixed_audio, mode='complex')
            speech_x = calc_sp(args, speech_audio, mode='magnitude')
            noise_x = calc_sp(args, noise_audio, mode='magnitude')

            #data = [mixed_complx_x, speech_x, noise_x, alpha]
...
def create_mixture_csv(args):
    ...
    rs = np.random.RandomState(0)
    out_csv_path = os.path.join(workspace, "datasave", "mixture_csvs", "%s.csv" % data_type)
...
def calculate_mixture_features(args):
    ...
    # Open mixture csv. 
    mixture_csv_path = os.path.join(workspace, "datasave", "mixture_csvs", "%s.csv" % data_type)
    ...
    for i1 in range(1, len(lis)):
        ...
        out_audio_path = os.path.join(workspace, "datasave", "mixed_audios", "spectrogram", 
        ...
        # Write out features. 
        out_feat_path = os.path.join(workspace, "datasave", "features", "spectrogram", 
...
def pack_features(args):
    ...
    # Load all features. 
    feat_dir = os.path.join(workspace, "datasave", "features", "spectrogram", data_type, "%ddb" % int(snr))
    ...
    out_path = os.path.join(workspace, "datasave", "packed_features", "spectrogram", data_type, "%ddb" % int(snr), "data.h5")
...
def compute_scaler(args):
    ...
    # Load data. 
    t1 = time.time()
    hdf5_path = os.path.join(workspace, "datasave", "packed_features", "spectrogram", data_type, "%ddb" % int(snr), "data.h5")
    ...
    # Write out scaler. 
    out_path = os.path.join(workspace, "datasave", "packed_features", "spectrogram", data_type, "%ddb" % int(snr), "scaler.p")
    create_folder(os.path.dirname(out_path))
...
def load_hdf5(hdf5_path):
    ...
    with h5py.File(hdf5_path, 'r') as hf:
        ...
        y = hf.get('y')
        x = np.array(x)     # (n_segs, n_concat, n_freq)
        y = np.array(y)     # (n_segs, n_freq)    => (n_segs, 1 ,n_freq)
        y = y[:, np.newaxis, :]
    return x, y
